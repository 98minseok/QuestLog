...
public class CurrentUserResolver {
    ...
    public static final CurrentUser DEVELOPMENT_USER =
            new CurrentUser("dev-user", "Quest Hero", "Asia/Seoul", null, false);
    ...
    public CurrentUser resolve() {
        ...
        if (principal instanceof Jwt jwt) {
            return fromAttributes(jwt.getClaims(), authentication.getName(), jwt.getTokenValue());
        }
        if (principal instanceof OAuth2AuthenticatedPrincipal oauthPrincipal) {
            return fromAttributes(oauthPrincipal.getAttributes(), authentication.getName(), null);
        }
    ...
    private CurrentUser fromAttributes(
        ...
        return new CurrentUser(subject, displayName, timezone, bearerToken, true);
    }
